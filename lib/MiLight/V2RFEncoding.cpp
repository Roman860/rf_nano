...
void V2RFEncoding::decodeV2Packet(uint8_t *packet) {
  uint8_t key = xorKey(packet[0+1]);

  for (size_t i = 1; i < 8; i++) // don't decode last byte (c.sum)
  {
    packet[i+1] = decodeByte(packet[i+1], 0, key, V2_OFFSET(i, packet[0+1], V2_OFFSET_JUMP_START));
  }  
}
...
uint8_t V2RFEncoding::checksumV2Packet(uint8_t *packet) {
  uint8_t key = xorKey(packet[1]);
  uint8_t sum = key;

  for (size_t i = 2; i <= 8; i++) 
    sum += packet[i];

  return(encodeByte(sum, 2, key, V2_OFFSET(8, packet[1], 0)));
}